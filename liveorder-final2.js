/* ══════════════════════════════════════════════════════════════
   LIVEORDER PRO — app.js v11.0
   Architecture : CRO-First · Mobile-First · Stripe uniquement
   Submit       : draft pending_payment → Formspree → redirection Stripe
   UX           : delivery selector · analytics hooks · premium reassurance
══════════════════════════════════════════════════════════════ */
'use strict';
...
const BASE_CONFIG = {
  ...
  shippingOptions: {
    mondial_relay: {
      ...
      reassuranceTitle: 'Option recommandée pour un retrait souple',
      reassuranceText: 'Mondial Relay est conseillé pour un meilleur équilibre entre simplicité et coût.'
    },
    chronopost: {
      code: 'chronopost',
      label: 'Chronopost · Domicile prioritaire',
      shortLabel: 'Chronopost',
      price: 8.90,
      eta: '24 à 48 h ouvrées',
      reassuranceTitle: 'Option prioritaire pour aller plus vite',
      reassuranceText: 'Chronopost est conseillé si vous voulez privilégier la vitesse de réception.'
    }
  },
  defaultShipping: 'mondial_relay',
  formspreeEndpoint: 'https://formspree.io/f/REPLACE_FORM_ID',
  stripeLink: 'https://buy.stripe.com/REPLACE_STRIPE_LINK',
  socialLinks: {
    snapchat: 'https://www.snapchat.com/add/REPLACE_SNAP',
    tiktok: 'https://www.tiktok.com/@REPLACE_TIKTOK',
    whatnot: 'https://www.whatnot.com/user/REPLACE_WHATNOT'
  },
...
const CONFIG = (() => {
  const runtime = window.__LIVEORDER_RUNTIME__ || {};
  return {
    ...BASE_CONFIG,
    ...runtime,
    socialLinks: { ...BASE_CONFIG.socialLinks, ...(runtime.socialLinks || {}) },
    shippingOptions: { ...BASE_CONFIG.shippingOptions, ...(runtime.shippingOptions || {}) }
  };
})();
...
const state = {
  ...
  isKnownRef: false,
  quantity: 1,
  selectedShipping: CONFIG.defaultShipping,
  orderIntentId: '',
  orderStatus: 'draft',
  isSubmitting: false,
  get shipping() {
    return CONFIG.shippingOptions[this.selectedShipping] || Object.values(CONFIG.shippingOptions)[0];
  },
  get subtotal() { return this.productPrice * this.quantity; },
  get total() { return this.productPrice > 0 ? this.subtotal + this.shipping.price : 0; },
...
const analytics = {
  push(event, payload = {}) {
    const data = { event, step: state.currentStep, shipping: state.selectedShipping, ...payload };
...
function ensureIntentId() {
  if (state.orderIntentId) return state.orderIntentId;
  state.orderIntentId = `lo_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
  return state.orderIntentId;
}

function syncRuntimeLinks() {
  const map = {
    sidebarStripeLink: CONFIG.stripeLink,
  };
  ...
  const socialMap = {
    '.social-card--snap': CONFIG.socialLinks.snapchat,
    '.social-card--tiktok': CONFIG.socialLinks.tiktok,
    '.social-card--whatnot': CONFIG.socialLinks.whatnot,
  };
  Object.entries(socialMap).forEach(([selector, href]) => {
    const el = document.querySelector(selector);
    if (el) el.href = href;
  });

  const form = $id('orderForm');
  if (form) form.action = CONFIG.formspreeEndpoint;
}
...
function updateSummarySheet() {
  const data = {
    ...
    mobSheetShipping: `${state.shipping.shortLabel} · ${fmt.euro(state.shipping.price)}`,
...
function syncShippingUI() {
  const selected = state.shipping;
  document.querySelectorAll('[data-shipping]').forEach((el) => {
    const active = el.getAttribute('data-shipping') === state.selectedShipping;
    el.classList.toggle('active', active);
    el.setAttribute('aria-checked', active ? 'true' : 'false');
  });

  const title = $id('shippingReassuranceTitle');
  const line = $id('shippingPrimaryLine');
  if (title) title.textContent = selected.reassuranceTitle;
  if (line) line.textContent = selected.reassuranceText;
}
...
function render() {
  ...
  const pairs = {
    ...
    summaryShipping: state.hasProduct ? `${state.shipping.shortLabel} · ${fmt.euro(state.shipping.price)}` : '—',
    ...
    sbShipping: state.hasProduct ? `${state.shipping.shortLabel} · ${fmt.euro(state.shipping.price)}` : '—',
    ...
    hiddenShipping: state.shipping.price.toFixed(2),
    hiddenShippingCode: state.shipping.code,
    hiddenShippingLabel: state.shipping.label,
    hiddenOrderStatus: state.orderStatus,
    hiddenOrderIntentId: ensureIntentId(),
...
function goToStep(n) {
  ...
  updateMobileBar(n);
  analytics.push('checkout_step_view', { checkout_step: n });
}
...
function initAutosave() {
  ...
  const fields = ['prenom','nom','email','telephone','adresse1','adresse2','ville','code_postal','pseudo','reference','prix_unitaire','notes'];
...
function initReset() {
  $id('resetAllBtn')?.addEventListener('click', () => {
    ...
    state.isKnownRef = false;
    state.quantity = 1;
    state.selectedShipping = CONFIG.defaultShipping;
...
function syncStripeLink() { syncRuntimeLinks(); }
...
function initShippingSelectors() {
  document.querySelectorAll('[data-shipping]').forEach((btn) => {
    btn.addEventListener('click', () => {
      const code = btn.getAttribute('data-shipping');
      if (!CONFIG.shippingOptions[code]) return;
      state.selectedShipping = code;
      analytics.push('shipping_option_selected', { shipping_code: code, shipping_price: state.shipping.price });
      render();
    });
  });
}
...
function initEvents() {
  ...
  $id('step1Next')?.addEventListener('click', () => {
    ...
    analytics.push('checkout_step_validated', { checkout_step: 1, reference: state.productRef, known_reference: state.isKnownRef });
    renderStep1Summary();
    goToStep(2);
  ...
  $id('step2Next')?.addEventListener('click', () => {
    ...
    analytics.push('checkout_step_validated', { checkout_step: 2 });
    renderStep2Summary();
    goToStep(3);
  ...
  $id('step3Next')?.addEventListener('click', () => {
    ...
    analytics.push('checkout_step_validated', { checkout_step: 3, shipping_code: state.selectedShipping });
    goToStep(4);
  ...
  $id('orderForm')?.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      analytics.push('checkout_payment_cta_clicked', { order_intent_id: state.orderIntentId, total: state.total, shipping_code: state.selectedShipping });
      ...
      analytics.push('checkout_redirect_stripe', { order_intent_id: state.orderIntentId, total: state.total });
      window.location.href = CONFIG.stripeLink;
    } catch (err) {
      analytics.push('checkout_submit_error', { order_intent_id: state.orderIntentId });
      state.orderStatus = 'draft';
      render();
      ...
      if (txt) txt.textContent = 'Continuer vers Stripe';
...
function init() {
  syncStripeLink();
  initShippingSelectors();
  analytics.push('hero_view');
  initRipple();
...
window.addEventListener('pagehide', () => {
  if (!state.isSubmitting && state.currentStep < 4) {
    analytics.push('checkout_abandon', { checkout_step: state.currentStep, order_intent_id: state.orderIntentId || null });
  }
});
