...
function validateStep(n) {
  ...
  if (n === 1) {
    ...
    if (!ref) {
      // BUG FIX : si ref vide, valider UNIQUEMENT la référence
      // (customPriceField est masqué → ne jamais montrer d'erreur prix ici)
      showErr('referenceError', 'Référence requise.');
      ok = false;
    } else {
      clearErr('referenceError');
      // Prix requis seulement si la référence n'est pas reconnue dans le catalogue
      if (!state.isKnownRef) {
...
function handleReference() {
  ...
  if (prod) {
    ...
    // BUG FIX : ne pas écraser le prix à 0 si un prix manuel a déjà été saisi
